<?php
/**
 * Rename script for siteStatistics
 *
...
function rename_extra($start_path, $find = array(), $replace = array()) {
	...
	foreach ($items as $item) {
		...
		$old_path = $start_path . '\\' . $item;

		if (strpos($old_path, $find[1]) !== false) {
			$new_path = $start_path . '\\' . str_replace($find, $replace, $item);
		...
		if (is_dir($new_path)) {
			...
			if ($item == 'index.class.php') {
				$content = str_replace($replace[0] . 'ManagerController', 'siteStatisticsManagerController', $content);
			}
